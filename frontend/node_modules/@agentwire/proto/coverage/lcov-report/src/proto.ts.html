...
<head>
    <title>Code coverage report for src/proto.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> proto.ts</h1>
        <div class='clearfix'>
...
<a name='L97'></a><a href='#L97'>97</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">6x</span>
...
<span class="cline-any cline-yes">62x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { BaseEvent, EventSchemas, EventType, Message } from "@agentwire/core";
...
export function encode(event: BaseEvent): Uint8Array {
  ...
  const { type, timestamp, threadId, runId, rawEvent, ...rest } = event as any;
&nbsp;
...
export function decode(data: Uint8Array): BaseEvent {
  ...
  decoded.threadId = decoded.baseEvent.threadId;
  decoded.runId = decoded.baseEvent.runId;
...
&nbsp;
  ...
  if (decoded.type === EventType.MESSAGES_SNAPSHOT) {
    for (const message of (decoded as any).messages as Message[]) {
      if (message.toolCalls<span class="branch-0 cbranch-no" title="branch not covered" >?.l</span>ength === 0) {
...
&nbsp;
  ...
  if (decoded.type === EventType.STATE_DELTA) {
    for (const operation of (decoded as any).delta) {
      operation.op =
        protoPatch.JsonPatchOperationType[operation.op].toLowerCase();
