...
def _is_newbuilding(offer: Dict[str, Any]) -> bool:
    ...
    NOTE: Main filtering is done in browser_fetcher.py which considers
    "дом сдан" status. Here we only check explicit markers.
    """
    # Check buildingStatus field (set by browser_fetcher.py)
    ...
    # Check category field (set by browser_fetcher.py)
