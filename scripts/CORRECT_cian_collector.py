#!/usr/bin/env python3
"""
ИСПРАВЛЕННЫЙ сборщик CIAN с правильными фильтрами.
Собирает только объявления, соответствующие заданным критериям.
"""
...
def extract_offers_with_filters(page) -> List[Dict[str, Any]]:
    """Извлекает объявления с проверкой соответствия фильтрам."""
    try:
...
def parse_and_validate_offer(offer: Dict[str, Any]) -> Dict[str, Any]:
    """Парсит и валидирует объявление по фильтрам."""
    ...
    # Проверка этажа
    if 'floor' in parsed_info and parsed_info['floor'] < MIN_FLOOR:
        validation_errors.append(f"Этаж {parsed_info['floor']} < {MIN_FLOOR}")
    
    # Проверка комнат
    if 'rooms' in parsed_info and parsed_info['rooms'] not in ALLOWED_ROOMS:
...
def save_valid_offers_to_db(offers: List[Dict[str, Any]]) -> int:
    ...
    LOGGER.info(f"💾 Сохранение {len(valid_offers)} валидных объявлений в БД...")
...
def collect_filtered_cian_data():
    """Собирает данные с CIAN с правильными фильтрами."""
    
    LOGGER.info("🚀 Запуск ИСПРАВЛЕННОГО сборщика CIAN с фильтрами...")
    LOGGER.info("📋 ФИЛЬТРЫ:")
    LOGGER.info("   💰 Цена: до 30 000 000 ₽")
    LOGGER.info("   🏢 Этаж: от 2")
    LOGGER.info("   🏠 Комнаты: 0, 1, 2, 3")
    LOGGER.info("   🏘️ Тип: вторичка")
    LOGGER.info("")
    
    # КОРРЕКТНЫЙ URL с фильтрами
    url = "https://www.cian.ru/cat.php?deal_type=sale&engine_version=2&offer_type=flat&region=1&building_status=secondary&price_min=1000000&price_max=30000000&floor_min=2&room=0&room=1&room=2&room=3&p=1"
    ...
    with sync_playwright() as p:
        ...
        try:
            LOGGER.info("📄 Загрузка страницы с фильтрами...")
            page.goto(url, wait_until="domcontentloaded", timeout=30000)
            time.sleep(5)
            
            # Извлекаем данные
            LOGGER.info("🔍 Извлечение данных...")
            offers_data = extract_offers_with_filters(page)
            
            if offers_data:
                LOGGER.info(f"✅ Извлечено {len(offers_data)} объявлений")
                
                # Валидируем каждое объявление
                for offer in offers_data:
                    validated_offer = parse_and_validate_offer(offer)
                    all_offers.append(validated_offer)
                    
                    if validated_offer.get('is_valid', False):
                        valid_offers.append(validated_offer)
                    else:
                        errors = validated_offer.get('validation_errors', [])
                        LOGGER.warning(f"❌ ID {validated_offer.get('id')}: {', '.join(errors)}")
                
                LOGGER.info(f"📊 РЕЗУЛЬТАТЫ ВАЛИДАЦИИ:")
                LOGGER.info(f"   Всего извлечено: {len(all_offers)}")
                LOGGER.info(f"   Соответствуют фильтрам: {len(valid_offers)}")
                LOGGER.info(f"   Не соответствуют: {len(all_offers) - len(valid_offers)}")
                ...
                # Сохраняем в файл
                output_file = "logs/CORRECT_cian_data.json"
                with open(output_file, 'w', encoding='utf-8') as f:
                    json.dump({
                        'collection_info': {
                            'type': 'CORRECT CIAN DATA - WITH FILTERS',
                ...
                LOGGER.error("❌ Данные не извлечены")
                return 0
...
if __name__ == "__main__":
    valid_count = collect_filtered_cian_data()
    
    if valid_count > 0:
        LOGGER.info(f"\n🎉 УСПЕХ: Собрано {valid_count} объявлений, соответствующих фильтрам!")
